document.addEventListener('DOMContentLoaded', () => {
    ...
    // On Load: This is now the ONLY place where the game starts.
    loadingManager.onLoad = () => {
        loadingContainer.style.display = 'none';
        console.log("Assets Loaded, starting the game!");
        animate(); // Start the game loop
    };

    // On Progress: Updates the loading bar
    loadingManager.onProgress = (url, itemsLoaded, itemsTotal) => {
        progressBar.style.width = `${(itemsLoaded / itemsTotal) * 100}%`;
    };

    // On Error: Catches any loading failures (e.g., file not found)
    loadingManager.onError = (url) => {
        errorScreen.style.display = 'block';
        loadingContainer.style.display = 'none';
        errorScreen.innerHTML = `<h2>Loading Failed</h2>
            <p>Could not load asset: <strong>${url}</strong></p>
            <p><strong>Troubleshooting:</strong></p>
            <ol style="text-align: left;">
                <li>Check your internet connection.</li>
                <li>Ensure the folder is named 'models' (lowercase).</li>
                <li>Ensure the file is named 'airplane.glb' (lowercase).</li>
                <li>Open the Developer Console for more details.</li>
            </ol>`;
    };
    ...
    // --- Lighting (Typo Fixed) ---
    // FIX: Changed '0add8e6' to the correct '0xadd8e6'
    ...
    loader.load('models/airplane.glb', 
        ...
        undefined, // onProgress is handled by the manager
        (error) => { // Redundant error callback for extra safety
            console.error("An error happened during model loading:", error);
            loadingManager.onError('models/airplane.glb'); // Trigger the main error handler
        }
    );
    ...
    const physics = { drag: 0.985, angularDrag: 0.98, liftCoefficient: 0.1, sensitivity: { pitch: 0.001, roll: 0.0008, yaw: 0.0005 }, stallAngle: 0.3 };
    ...
    // --- GAME LOOP (Now only called by loadingManager.onLoad) ---
    function animate() {
        ...
        const velocityDirection = airspeed > 0.01 ? localPlayer.velocity.clone().normalize() : forwardVector;
        ...
        if (localPlayer.mesh.position.y < GROUND_LEVEL + 5) liftMagnitude *= 1.2;
    ...
    // NOTE: The animate() function is NO LONGER called here. It's now called inside loadingManager.onLoad.
